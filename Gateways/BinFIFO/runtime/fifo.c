...
static uint16_t num_valid_sockets;
...
static void printPayload(char *payload, size_t size) {
	for (size_t i=0; i < size; i++) {
		if (!isprint(payload[i])) { printf("\033[40;32;1m.\033[0m"); }
		putchar(payload[i]);
	}
	putchar('\n');
}
...
static void process(max_framed_stream_t *toCpuStream, max_framed_stream_t *outboundStream) {
	...
	while (max_framed_stream_read(toCpuStream, 1, &frame, &frame_size) == 1) {
		...
		printf("socket[%u] Event: ConnStateValid %d, errorCode %d, containsData: %d\n", socket_num, 
				header->connectionStateValid, header->errorCode, header->containsData);

		if (socket_num < num_valid_sockets) {
			...
			if (header->containsData) {
				...
				if (frame_size > sizeof(struct InboundHeaderType_s)) {
					...
					printf("socket[%u] Got payload (%zd bytes): ", socket_num, payloadSize);
