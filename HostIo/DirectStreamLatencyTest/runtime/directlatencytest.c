...
int main(int argc, char *argv[]) {
	...
	max_direct_stream_t *src_stream = max_direct_stream_init(engine, "src");
	...
	for (size_t i=0; i < iterations; i++) {
		...
		uint8_t *src_slot_data = src_buffer;
		...
		max_direct_stream_write(src_stream, src_slot_data, slot_size_bytes);
		...
		// Compare the data
		if (memcmp(src_slot_data, dst_slot, slot_size_bytes)) {
